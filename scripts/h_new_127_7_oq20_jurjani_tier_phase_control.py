#!/usr/bin/env python3
"""H-NEW-127.7 - phase-aware control for the Jurjani-tier bridge.
...
Outer null:
  shuffle tier labels only within neuwirth_phase blocks, preserving the tier
  counts inside each phase block, and recompute the global H
"""
...
PREREG_FILE = ROOT / "findings/phase-b-hypotheses/h-new-127-7-jurjani-tier-phase-aware-control-prereg.md"
...
OUT_JSON = ROOT / "findings/phase-b-hypotheses/csv/h-new-127-7.json"
...
def main() -> int:
    ...
    for idx, rec in enumerate(records):
        tier = str(rec["jurjani_predicted_asyndeton_tier"])
        phase = str(rec["phase"])
        z_s = float(rec["z_s"])
        tier_to_scores[tier].append(z_s)
        phase_to_counts[phase][tier] += 1
    ...
    tier_labels = [str(rec["jurjani_predicted_asyndeton_tier"]) for rec in records]
    rng = random.Random(SEED)
    h_null = []
    n_ge = 0
    for _ in range(N_PERM):
        perm_labels = tier_labels[:]
        ...
        for rec, tier in zip(records, perm_labels):
            perm_groups[tier].append(float(rec["z_s"]))
        ...
        phase_summary.append(
            {
                "phase": phase,
                "n": len(vals),
                "tiers_present": sorted(phase_to_counts[phase]),
                "n_tiers_present": len(phase_to_counts[phase]),
                "mean_z_s": statistics.mean(vals),
                "median_z_s": statistics.median(vals),
            }
        )
    ...
    for phase in sorted(phase_to_counts):
        row = {
            ...
            "tier_counts": {tier: phase_to_counts[phase][tier] for tier in tiers_sorted if phase_to_counts[phase][tier] > 0},
        }
    ...
    out = {
        ...
        "control_axis": {
            ...
            "note": "Tier labels are shuffled only within these phase blocks, preserving the observed tier counts inside each phase.",
        },
