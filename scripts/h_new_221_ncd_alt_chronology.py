#!/usr/bin/env python3
"""H-NEW-221 — Cross-feature replication of H-NEW-212 under NCD feature space.
...
D-matrix: NCD-lzma from H-NEW-169 (preset=9|EXTREME, 0x00 separator,
arithmetic-mean symmetrization). Loaded from csv/h-new-169-ncd-matrix.npy
(114x114, 0-indexed so surah k -> row k-1).

Bell 1937 / Blachère 1947 rank tables imported verbatim from the
H-NEW-212 script (same ties, same imputations).

Seed 20260419. 10000 perms.
"""
import csv
import hashlib
import json
...
PREREG_FILE = ROOT / 'findings/phase-b-hypotheses/h-new-221-ncd-alt-chronology-prereg.md'
NCD_NPY = ROOT / 'findings/phase-b-hypotheses/csv/h-new-169-ncd-matrix.npy'
H169_JSON = ROOT / 'findings/phase-b-hypotheses/csv/h-new-169.json'
H212_SCRIPT = ROOT / 'scripts/h_new_212_alt_chronology_fisher_rao.py'
NOLDEKE_CSV = ROOT / 'data/revelation-order.csv'
OUT_JSON = ROOT / 'findings/phase-b-hypotheses/csv/h-new-221.json'

# -----------------------------------------------------------------------------
# Pre-reg tamper-evidence
# -----------------------------------------------------------------------------
prereg_sha = hashlib.sha256(PREREG_FILE.read_bytes()).hexdigest()
ncd_sha = hashlib.sha256(NCD_NPY.read_bytes()).hexdigest()
h169_sha = hashlib.sha256(H169_JSON.read_bytes()).hexdigest()
h212_script_sha = hashlib.sha256(H212_SCRIPT.read_bytes()).hexdigest()
print(f"prereg SHA-256:              {prereg_sha}", file=sys.stderr)
print(f"NCD .npy SHA-256:            {ncd_sha}", file=sys.stderr)
print(f"h-new-169.json SHA-256:      {h169_sha}", file=sys.stderr)
print(f"h-new-212 script SHA-256:    {h212_script_sha}", file=sys.stderr)
print(f"SEED = {SEED}", file=sys.stderr)
print(f"PERMS = {PERMS}", file=sys.stderr)
print(f"BONFERRONI_K = {BONFERRONI_K}, α_bon = {ALPHA_BON:.5f}", file=sys.stderr)

# -----------------------------------------------------------------------------
# 1. Load D matrix from H-NEW-169 NCD
# -----------------------------------------------------------------------------
print("\nLoading NCD D matrix from h-new-169-ncd-matrix.npy ...", file=sys.stderr)
D = np.load(NCD_NPY)
assert D.shape == (114, 114), f"bad D shape {D.shape}"
# 0-indexed: surah k (1..114) -> row k-1
assert np.allclose(np.diagonal(D), 0.0), "D diagonal should be zero"
# symmetric? D-matrix from H-NEW-169 is arithmetic-mean symmetrized
asym = float(np.max(np.abs(D - D.T)))
print(f"  D shape = {D.shape}, diag=0, max|D - D.T| = {asym:.2e}", file=sys.stderr)

# Reference values from H-NEW-169 JSON
h169 = json.loads(H169_JSON.read_text())
L_mushaf_h169 = float(h169['L_mushaf'])


def path_length(order_1indexed):
    """order_1indexed: list of surah IDs 1..114. Converts to 0-indexed
    row access."""
    L = 0.0
    for i in range(len(order_1indexed) - 1):
        a = order_1indexed[i] - 1
        b = order_1indexed[i + 1] - 1
        L += D[a, b]
    return L


# Sanity: recompute L_mushaf and compare
mushaf_order = list(range(1, 115))
L_mushaf = path_length(mushaf_order)
matches_h169 = abs(L_mushaf - L_mushaf_h169) < 1e-6
print(f"  recomputed L_mushaf = {L_mushaf:.6f}  (matches H-NEW-169 "
      f"{L_mushaf_h169:.6f}: {matches_h169})", file=sys.stderr)
assert matches_h169, "L_mushaf mismatch vs H-NEW-169"
...
print(f"\nL_egyptian = {L_egyptian:.6f}", file=sys.stderr)
print(f"L_noldeke  = {L_noldeke:.6f}", file=sys.stderr)

# -----------------------------------------------------------------------------
# 3. Bell 1937 / Blachère 1947 rank tables (copied verbatim from H-NEW-212
#    script; see data-quality caveats in prereg).
# -----------------------------------------------------------------------------
BELL_RANK = {
...
assert len(BELL_RANK) == 114
assert len(BLACHERE_RANK) == 114
for sid in range(1, 115):
    assert sid in BELL_RANK
    assert sid in BLACHERE_RANK

# Tie-break: mushaf-order ascending
...
print(f"L_bell      = {L_bell:.6f}", file=sys.stderr)
print(f"L_blachere  = {L_blachere:.6f}", file=sys.stderr)

# -----------------------------------------------------------------------------
# 4. Fresh null: 10,000 uniform permutations, seed 20260419
# -----------------------------------------------------------------------------
...
print(f"  null min={null_sorted[0]:.6f} q05={null_quantiles['q05']:.6f}",
      file=sys.stderr)
...
for name, L, order_list in [
    ...
    print(f"  {name:15s}: L={L:.6f}  z={z:+.3f}  p_1sided={p:.6f}",
          file=sys.stderr)
...
for rank, (name, r) in enumerate(leaderboard, 1):
    ...
    print(f"  {rank}. {name:15s} L={r['L']:.6f}  p={r['p_one_sided_lower']:.6f}{tag}",
          file=sys.stderr)
...
# -----------------------------------------------------------------------------
# 7. Bonferroni verdicts (3-member family)
# -----------------------------------------------------------------------------
...
for name in family:
    ...
    passed = p < ALPHA_BON
    verdicts[name] = {'p': p, 'alpha_bon': ALPHA_BON, 'pass': passed}
    if passed:
        any_pass = True
...
# -----------------------------------------------------------------------------
# 8. Spearman rank correlations across chronologies (diagnostic; rank data
#    is identical to H-NEW-212 — values should match.)
# -----------------------------------------------------------------------------
...
for name in ['egyptian_1924', 'noldeke_1860', 'bell_1937', 'blachere_1947']:
    ...
    print(f"  {name}: Δ={d:+.6f} ({d/null_sd:+.3f} SDs)  [{sign}]",
          file=sys.stderr)

# -----------------------------------------------------------------------------
# 10. Cross-feature concordance with H-NEW-212 (Fisher-Rao)
# -----------------------------------------------------------------------------
h212_json_path = ROOT / 'findings/phase-b-hypotheses/csv/h-new-212.json'
h212 = json.loads(h212_json_path.read_text())
fr_leaderboard = [row['name'] for row in h212['leaderboard_shortest_first']]
ncd_leaderboard = [name for name, _ in leaderboard]
concordance_identical = (fr_leaderboard == ncd_leaderboard)

# Spearman over the two leaderboards (position of same name in each)
pos_fr = {n: i for i, n in enumerate(fr_leaderboard)}
pos_ncd = {n: i for i, n in enumerate(ncd_leaderboard)}
common = [n for n in pos_fr if n in pos_ncd]
a = [pos_fr[n] for n in common]
b = [pos_ncd[n] for n in common]
# Simple Spearman (positions are already ranks 0..4 unique)
mean_a = sum(a) / len(a)
mean_b = sum(b) / len(b)
num = sum((a[i] - mean_a) * (b[i] - mean_b) for i in range(len(a)))
da = math.sqrt(sum((a[i] - mean_a) ** 2 for i in range(len(a))))
db = math.sqrt(sum((b[i] - mean_b) ** 2 for i in range(len(b))))
leaderboard_spearman = num / (da * db) if da * db > 0 else 0.0

print(f"\nCross-feature (NCD vs Fisher-Rao) leaderboard concordance:",
      file=sys.stderr)
print(f"  FR:  {fr_leaderboard}", file=sys.stderr)
print(f"  NCD: {ncd_leaderboard}", file=sys.stderr)
print(f"  identical? {concordance_identical}", file=sys.stderr)
print(f"  Spearman ρ (leaderboard positions) = {leaderboard_spearman:+.4f}",
      file=sys.stderr)
...
summary = {
    ...
    'title': ('Cross-feature replication of H-NEW-212 under NCD feature '
              'space (non-parametric)'),
    'parent_lineage': ['H-NEW-169', 'H-NEW-212', 'cross-finding-011'],
    'pre_reg_sha256': prereg_sha,
    'ncd_npy_sha256': ncd_sha,
    'h_new_169_json_sha256': h169_sha,
    'h_new_212_script_sha256': h212_script_sha,
    'seed': SEED,
    'permutations': PERMS,
    'bonferroni_k': BONFERRONI_K,
    'alpha_bon': ALPHA_BON,
    'date': '2026-04-17',
    'feature_space': 'NCD-lzma (preset=9|EXTREME, 0x00 sep, arith-mean sym)',
    'rules_tuple': ('(no-tashkeel, QAC v0.4, basmala-counted-only-in-surah-1, '
                    'Hafs-Kufan, D-matrix-inherited-from-H-NEW-169 NCD)'),
    ...
    'cross_feature_concordance_with_h212': {
        'fisher_rao_leaderboard': fr_leaderboard,
        'ncd_leaderboard': ncd_leaderboard,
        'identical': concordance_identical,
        'leaderboard_spearman': leaderboard_spearman,
    },
    ...
    'verdict_primary': 'SEE leaderboard + family verdicts',
}
...
# -----------------------------------------------------------------------------
# 12. Final stderr summary
# -----------------------------------------------------------------------------
print("\n" + "=" * 72, file=sys.stderr)
print("H-NEW-221 SUMMARY (NCD-lzma; 3-chronology family; α_bon=0.01667)",
      file=sys.stderr)
print("=" * 72, file=sys.stderr)
print(f"  L_mushaf        = {L_mushaf:.6f}  (reference, not in family)",
      file=sys.stderr)
print(f"  L_noldeke_1860  = {L_noldeke:.6f}  (reference, not in family)",
      file=sys.stderr)
print(f"  L_egyptian_1924 = {L_egyptian:.6f}  p={results['egyptian_1924']['p_one_sided_lower']:.6f}",
      file=sys.stderr)
print(f"  L_bell_1937     = {L_bell:.6f}  p={results['bell_1937']['p_one_sided_lower']:.6f}",
      file=sys.stderr)
print(f"  L_blachere_1947 = {L_blachere:.6f}  p={results['blachere_1947']['p_one_sided_lower']:.6f}",
      file=sys.stderr)
print(f"  null mean/sd = {null_mean:.6f} / {null_sd:.6f}", file=sys.stderr)
...
print(f"  mushaf still wins over all 3 chronologies: {mushaf_still_wins}",
      file=sys.stderr)
print(f"  cross-feature concordance with H-NEW-212: "
      f"identical={concordance_identical}, ρ={leaderboard_spearman:+.4f}",
      file=sys.stderr)
print("=" * 72, file=sys.stderr)
