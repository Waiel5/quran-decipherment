#!/usr/bin/env python3
"""H-NEW-220 — Cross-feature replication of H-NEW-212 under CHAR-4-GRAM.

Pre-registered tests (Bonferroni k=3, α_bon=0.01667):
  PRIMARY-1 — L_egyptian < L_random (1-sided lower)
  PRIMARY-2 — L_bell     < L_random (1-sided lower)
  PRIMARY-3 — L_blachere < L_random (1-sided lower)

Mushaf and Nöldeke reported descriptively (not in family), matching H-212.

Reuses distance matrix D from h-new-111b.json (Fisher-Rao angular distance
on char-4-gram top-2000 Dirichlet-smoothed L1-normalized probability
vectors).  Seed 20260419 (same as H-NEW-212).
...
PREREG_FILE = ROOT / 'findings/phase-b-hypotheses/h-new-220-alt-chronology-char4gram-prereg.md'
H111B_JSON = ROOT / 'findings/phase-b-hypotheses/csv/h-new-111b.json'
NOLDEKE_CSV = ROOT / 'data/revelation-order.csv'
OUT_JSON = ROOT / 'findings/phase-b-hypotheses/csv/h-new-220.json'

# Tamper-evidence
prereg_sha = hashlib.sha256(PREREG_FILE.read_bytes()).hexdigest()
h111b_sha = hashlib.sha256(H111B_JSON.read_bytes()).hexdigest()
print(f"prereg SHA-256:        {prereg_sha}", file=sys.stderr)
print(f"h-new-111b JSON SHA-256 (D matrix source): {h111b_sha}", file=sys.stderr)
print(f"SEED = {SEED}", file=sys.stderr)
print(f"PERMS = {PERMS}", file=sys.stderr)
print(f"BONFERRONI_K = {BONFERRONI_K}, α_bon = {ALPHA_BON:.5f}", file=sys.stderr)

# ----------------------------------------------------------------------
# 1. Load D matrix from h-new-111b.json (char-4-gram feature space)
# ----------------------------------------------------------------------
print("\nLoading D matrix from h-new-111b.json ...", file=sys.stderr)
h111b = json.loads(H111B_JSON.read_text())
D_up = h111b['D_matrix_upper_triangular']
D = [[0.0] * 115 for _ in range(115)]
for i, j, d in D_up:
    D[i][j] = float(d)
    D[j][i] = float(d)
print(f"  loaded {len(D_up)} pairs (expected 6441)", file=sys.stderr)
assert len(D_up) == 114 * 113 // 2, f"D upper-tri wrong size: {len(D_up)}"

L_mushaf_ref = float(h111b['primary']['L_mushaf'])
L_noldeke_ref = float(h111b['secondary_B']['L_noldeke'])
L_egyptian_ref = float(h111b['secondary_B']['L_tanzil_egyptian_std'])
print(f"  H-NEW-111b L_mushaf ref = {L_mushaf_ref:.4f}", file=sys.stderr)
print(f"  H-NEW-111b L_noldeke ref = {L_noldeke_ref:.4f}", file=sys.stderr)
print(f"  H-NEW-111b L_egyptian ref = {L_egyptian_ref:.4f}", file=sys.stderr)
...
assert abs(L_mushaf - L_mushaf_ref) < 1e-4, (
    f"L_mushaf recompute mismatch: {L_mushaf} vs {L_mushaf_ref}")
print(f"  recomputed L_mushaf = {L_mushaf:.4f}  (sanity OK)", file=sys.stderr)

# ----------------------------------------------------------------------
# 2. Load Egyptian + Nöldeke chronologies
# ----------------------------------------------------------------------
mushaf_to_egyptian = {}
mushaf_to_noldeke = {}
...
# Sanity vs h-new-111b stored values
assert abs(L_egyptian - L_egyptian_ref) < 1e-4, (
    f"L_egyptian mismatch: {L_egyptian} vs {L_egyptian_ref}")
assert abs(L_noldeke - L_noldeke_ref) < 1e-4, (
    f"L_noldeke mismatch: {L_noldeke} vs {L_noldeke_ref}")
print(f"\nL_egyptian = {L_egyptian:.4f}", file=sys.stderr)
print(f"L_noldeke  = {L_noldeke:.4f}", file=sys.stderr)

# ----------------------------------------------------------------------
# 3. Bell / Blachère rank dicts (verbatim copy from H-NEW-212 script)
# ----------------------------------------------------------------------
BELL_RANK = {
    1: 45, 2: 91, 3: 97, 4: 100, 5: 114, 6: 89, 7: 87, 8: 95, 9: 113, 10: 84,
    11: 75, 12: 77, 13: 90, 14: 76,
    15: 52,  # IMPUTED (middle-Meccan median; source coded "M")
    ...
    79: 20, 80: 17,
    81: 15, 82: 15,  # TIE (mushaf-order 2ndary)
...
BLACHERE_RANK = {
    ...
    80: 24, 84: 24,  # TIE (mushaf-order 2ndary)
    81: 82, 82: 86,
...
for sid in range(1, 115):
    assert sid in BELL_RANK and sid in BLACHERE_RANK
...
# ----------------------------------------------------------------------
# 4. Null: 10,000 uniform perms, seed 20260419
# ----------------------------------------------------------------------
...
print(f"  null min={null_sorted[0]:.4f}", file=sys.stderr)
...
# ----------------------------------------------------------------------
# 6. Bonferroni verdicts (3-member family)
# ----------------------------------------------------------------------
...
mushaf_beats_noldeke = results['noldeke_1860']['L'] > L_mushaf
mushaf_beats_all_4 = mushaf_still_wins and mushaf_beats_noldeke
print(f"\nMushaf shorter than all 3 family chronologies? {mushaf_still_wins}",
      file=sys.stderr)
print(f"Mushaf shorter than Nöldeke? {mushaf_beats_noldeke}",
      file=sys.stderr)
print(f"Mushaf rank-1 over all 4 chronologies? {mushaf_beats_all_4}",
      file=sys.stderr)


# ----------------------------------------------------------------------
# 7. Spearman diagnostics
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 8. Pairwise L_c − L_mushaf in null-SD units
# ----------------------------------------------------------------------
pairwise_diffs = {}
...
# ----------------------------------------------------------------------
# 9. Cross-finding generalization verdict
# ----------------------------------------------------------------------
if mushaf_beats_all_4:
    cross_verdict = ("H-NEW-212 and cross-finding-011 GENERALIZE to "
                     "char-4-gram feature space — mushaf rank 1 of 5 "
                     "under an orthogonal surface-form feature")
elif mushaf_still_wins:
    cross_verdict = ("PARTIAL generalization: mushaf beats the 3 "
                     "Bonferroni-family chronologies under char-4-gram "
                     "but LOSES to Nöldeke (descriptive reference). "
                     "Signal is mostly feature-invariant but Nöldeke-"
                     "specific comparison is feature-sensitive")
else:
    cross_verdict = ("FEATURE-SPECIFIC: at least one chronology beats "
                     "mushaf under char-4-gram — H-NEW-212 signal does "
                     "NOT fully generalize beyond QAC-root features")
print(f"\n{cross_verdict}", file=sys.stderr)
...
summary = {
    'finding_id': 'h-new-220',
    'title': ('Cross-feature replication of H-NEW-212 — '
              '4 chronologies vs mushaf under char-4-gram'),
    'parent_finding': 'h-new-212',
    'companion_finding': 'cross-finding-011',
    'feature_space_source': 'h-new-111b',
    'pre_reg_sha256': prereg_sha,
    'h_new_111b_source_sha256': h111b_sha,
    'seed': SEED,
    'permutations': PERMS,
    'bonferroni_k': BONFERRONI_K,
    'alpha_bon': ALPHA_BON,
    'date': '2026-04-17',
    'rules_tuple': ('(no-tashkeel, char-4-grams with spaces K=2000, '
                    'Dirichlet α=0.5, L1-normalized, Fisher-Rao angular, '
    ...
    'mushaf_beats_noldeke': mushaf_beats_noldeke,
    'mushaf_rank_1_over_all_4_chronologies': mushaf_beats_all_4,
    'chronology_spearman_matrix': rank_matrix,
    'pairwise_path_diffs_vs_mushaf': pairwise_diffs,
    'bell_data_quality_caveat': (
        'Surah 15 imputed rank 52 (coded "M" in Fr. Wiki); ties '
        '(s81/s82 rank 15) resolved by mushaf-order secondary'),
    'blachere_data_quality_caveat': (
        'Tie s80/s84 (both rank 24) resolved by mushaf-order secondary'),
    'cross_feature_generalization_verdict': cross_verdict,
...
# ----------------------------------------------------------------------
print("\n" + "=" * 72, file=sys.stderr)
print("H-NEW-220 SUMMARY (char-4-gram cross-feature check)", file=sys.stderr)
print("=" * 72, file=sys.stderr)
print(f"  L_mushaf        = {L_mushaf:.4f}", file=sys.stderr)
print(f"  L_noldeke_1860  = {L_noldeke:.4f}", file=sys.stderr)
...
print(f"  SHORTEST        = {shortest_name}", file=sys.stderr)
print(f"  mushaf rank     = {mushaf_rank} of 5", file=sys.stderr)
print(f"  beats all 4     = {mushaf_beats_all_4}", file=sys.stderr)
print(f"  verdict         = {cross_verdict}", file=sys.stderr)
print("=" * 72, file=sys.stderr)
