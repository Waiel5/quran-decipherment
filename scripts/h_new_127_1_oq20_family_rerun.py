#!/usr/bin/env python3
"""H-NEW-127.1 - Q55-repaired OQ-20 family rerun.

Family lock:
  - Q2/Q7/Q12/Q36: uniform full-verse permutation null, as in H-NEW-127
  - Q55: fixed refrain slots, permuting only the 47 non-refrain verses
          across the 47 non-refrain slots, as in H-NEW-280
...
PREREG_FILE = ROOT / "findings/phase-b-hypotheses/h-new-127-1-rerun-prereg.md"
OUT_JSON = ROOT / "findings/phase-b-hypotheses/csv/h-new-127-1.json"
OUT_JOURNAL = ROOT / "journal/h-new-127-1-run-1.md"
...
def main():
    ...
    for sdata in quran:
        ...
        print(f"  surah {sid}: n_verses={n_v}, mean root tokens/verse={sum(token_counts)/n_v:.2f}", file=sys.stderr)
    ...
    for sid in LOCKED_SURAHS:
        ...
        if sid == Q55_SURAH:
            q55 = locked[sid]["verses"]
            refrain_norm = norm(q55[12]["text"])
            refrain_positions = [verse["id"] for verse in q55 if norm(verse["text"]) == refrain_norm]
            ...
            refrain_slots = [pos - 1 for pos in refrain_positions]
            non_refrain_slots = [pos - 1 for pos in range(1, len(q55) + 1) if pos not in refrain_positions]
            ...
            for perm_idx in range(PERMS):
                ...
                order = [None] * len(q55)
                for slot, verse_idx in zip(refrain_slots, refrain_slots):
    ...
    result = {
        ...
        "locked_params": {
            ...
            "q55_null": "fixed_refrain_slot",
            "q55_refrain_positions": EXPECTED_REFRAIN_POSITIONS,
        },
        "corpus_stats": {
    ...
    journal = f"""# H-NEW-127.1 run 1 journal
...
- Greedy-NN shorter than canonical on all five surahs: `""" + str(all(control_rows[sid]["greedy_pass"] for sid in LOCKED_SURAHS)) + """`
- 2-opt shorter than canonical on all five surahs: `""" + str(all(control_rows[sid]["two_opt_pass"] for sid in LOCKED_SURAHS)) + """`
