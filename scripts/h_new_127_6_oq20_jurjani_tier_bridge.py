#!/usr/bin/env python3
"""H-NEW-127.6 - Jurjani-tier bridge for locked per-surah compression z-scores.

Observable:
  z_s = -gzip_z from the locked compression summary JSON

Primary test:
  Kruskal-Wallis H across the locked jurjani_predicted_asyndeton_tier labels

Outer null:
  shuffle tier labels across surahs, preserving class counts, and recompute H
"""
...
PREREG_FILE = ROOT / "findings/phase-b-hypotheses/h-new-127-6-jurjani-tier-bridge-prereg.md"
...
OUT_JSON = ROOT / "findings/phase-b-hypotheses/csv/h-new-127-6.json"
...
def main() -> int:
    ...
    for sid in range(1, 115):
        ...
        if not tier:
            raise RuntimeError(f"missing jurjani_predicted_asyndeton_tier for surah {sid}")
        records.append(
            {
                "surah": sid,
                ...
                "jurjani_predicted_asyndeton_tier": tier,
    ...
    tier_to_scores: dict[str, list[float]] = defaultdict(list)
    for rec in records:
        tier_to_scores[str(rec["jurjani_predicted_asyndeton_tier"])].append(float(rec["z_s"]))

    tiers_sorted = sorted(tier_to_scores)
    groups = [tier_to_scores[tier] for tier in tiers_sorted]
    h_obs, df = kruskal_wallis(groups)

    label_sequence = [str(rec["jurjani_predicted_asyndeton_tier"]) for rec in records]
    ...
    for _ in range(N_PERM):
        ...
        for z_val, tier in zip(z_values, perm_labels):
            perm_groups[tier].append(z_val)
        perm_h, _ = kruskal_wallis([perm_groups[tier] for tier in tiers_sorted])
    ...
    tier_summary.sort(key=lambda row: (row["median_z_s"], row["mean_z_s"], row["n"], row["tier"]))
    ...
    out = {
        ...
        "label_axis": {
            ...
            "field": "jurjani_predicted_asyndeton_tier",
            "note": "No relabeling or pooling; the exact locked tier taxonomy is used as-is.",
        },
        ...
        "n_perm": N_PERM,
        "alpha": ALPHA,
        "class_counts": {tier: len(vals) for tier, vals in tier_to_scores.items()},
        "primary_test": {
    ...
    print("H-NEW-127.6 - Jurjani-tier bridge for locked per-surah compression z-scores", file=sys.stderr)
    print(f"  surahs = {len(records)}", file=sys.stderr)
    print(f"  tiers = {len(tiers_sorted)}", file=sys.stderr)
    print(f"  H_obs = {h_obs:.6f} (df={df})", file=sys.stderr)
    print(f"  null mean = {null_mean:.6f}, sd = {null_sd:.6f}", file=sys.stderr)
    print(f"  p_perm_upper = {p_perm:.6f}", file=sys.stderr)
    print(f"  verdict = {verdict}", file=sys.stderr)
    print("  tiers by median z_s:", file=sys.stderr)
    for row in sorted(tier_summary, key=lambda r: (-r["median_z_s"], -r["mean_z_s"], -r["n"], r["tier"])):
        print(
            f"    {row['tier']}: n={row['n']}, median={row['median_z_s']:.6f}, mean={row['mean_z_s']:.6f}",
            file=sys.stderr,
        )
