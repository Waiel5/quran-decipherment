#!/usr/bin/env python3
"""H-NEW-127.11 - pooled within-phase rank test for residual Jurjani-tier signal.
...
Primary statistic:
  ...
  jurjani_predicted_asyndeton_tier computed separately inside each informative
  neuwirth_phase block

Informative phase rule:
  a phase with at least two distinct tier classes and at least one class count
  > 1

Outer null:
  shuffle tier labels only within each informative phase block, preserving the
  exact observed counts inside that block, and recompute T
"""
...
PREREG_FILE = ROOT / "findings/phase-b-hypotheses/h-new-127-11-pooled-within-phase-tier-rank-prereg.md"
...
OUT_JSON = ROOT / "findings/phase-b-hypotheses/csv/h-new-127-11.json"
...
def summarize_block(records: list[dict[str, object]], counts: Counter[str]) -> dict[str, object]:
    return {
        "n": len(records),
        "n_tier_classes": len(counts),
        "tier_counts": {tier: counts[tier] for tier in sorted(counts)},
...
def main() -> int:
    ...
    phase_tier_order: dict[str, list[str]] = {}
    t_obs = 0.0
    df_total = 0
    ...
    for phase in informative_phases:
        ...
        tiers = sorted(counts)
        groups = [
            [float(rec["z_s"]) for rec in block if str(rec["jurjani_predicted_asyndeton_tier"]) == tier]
            for tier in tiers
        ]
        h_obs, df = kruskal_wallis(groups)
        observed_by_phase[phase] = h_obs
        phase_labels[phase] = [str(rec["jurjani_predicted_asyndeton_tier"]) for rec in block]
        phase_z_values[phase] = [float(rec["z_s"]) for rec in block]
        phase_tier_order[phase] = tiers
        t_obs += h_obs
        df_total += df
    ...
    for _ in range(N_PERM):
        ...
        for phase in informative_phases:
            ...
            for z_s, tier in zip(phase_z_values[phase], labels_phase):
                perm_groups[tier].append(z_s)
            perm_h, _ = kruskal_wallis([perm_groups[tier] for tier in phase_tier_order[phase]])
            perm_t += perm_h
    ...
    for phase in phases_sorted:
        ...
        excluded_blocks.append(
            {
                "phase": phase,
                "exclusion_reason": "single-class" if len(counts) == 1 else "all-singletons",
    ...
    out = {
        ...
        "label_axis": {
            ...
            "note": "No relabeling or manual pooling; the exact locked Jurjani-tier labels are used as-is.",
        },
        "conditioning_axis": {
            "source_file": "findings/classical-sources/neuwirth-sinai-genre-labels.tsv",
