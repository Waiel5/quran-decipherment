#!/usr/bin/env python3
"""H-NEW-271.5: empirical-table minimal singleton rescue over H-NEW-271."""
...
OUT_JSON = ROOT / "findings/phase-b-hypotheses/csv/h-new-271-5.json"
...
def verdict_for(best_hits: int, p_maxT: float) -> str:
    if best_hits >= FULL_RESTORE_BAR and p_maxT < ALPHA:
        return "EMPIRICAL-H232-LEVEL-RESTORED"
    if best_hits >= PARTIAL_RESCUE_BAR and p_maxT < ALPHA:
        return "EMPIRICAL-SIGNIFICANT-PARTIAL-RESCUE"
    return "NO-MAXT-EMPIRICAL-RESCUE"
...
def main() -> None:
    ...
    out = {
        ...
        "accepted_sets_source": "H-NEW-274 empirical singleton table",
