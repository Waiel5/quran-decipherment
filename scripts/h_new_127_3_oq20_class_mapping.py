...
Primary test:
  Kruskal-Wallis H across the locked sinai_genre labels

Outer null:
  shuffle genre labels across surahs, preserving class counts, and recompute H
"""
...
PREREG_FILE = ROOT / "findings/phase-b-hypotheses/h-new-127-3-compression-class-mapping-prereg.md"
...
OUT_JSON = ROOT / "findings/phase-b-hypotheses/csv/h-new-127-3.json"
...
def main() -> int:
    ...
    label_axis = "sinai_genre"
    label_to_scores: dict[str, list[float]] = defaultdict(list)
    for rec in records:
        label_to_scores[rec["label"]].append(rec["z_s"])

    labels_sorted = sorted(label_to_scores)
    groups = [label_to_scores[label] for label in labels_sorted]
    h_obs, df = kruskal_wallis(groups)

    label_sequence = [rec["label"] for rec in records]
    ...
    for _ in range(N_PERM):
        ...
        for z_val, lab in zip(z_values, perm_labels):
            perm_groups[lab].append(z_val)
        perm_h, _ = kruskal_wallis([perm_groups[label] for label in labels_sorted])
    ...
    label_summary.sort(key=lambda row: (row["median_z_s"], row["mean_z_s"], row["n"], row["label"]))
    ...
    out = {
        ...
        "label_axis": {
            ...
            "field": label_axis,
            "note": "No label pooling; the exact locked genre taxonomy is used as-is.",
        },
        ...
        "n_perm": N_PERM,
        "alpha": ALPHA,
        "class_counts": {label: len(vals) for label, vals in label_to_scores.items()},
        "primary_test": {
    ...
    print("H-NEW-127.3 - class-mapping of locked per-surah compression z-scores", file=sys.stderr)
    print(f"  surahs = {len(records)}", file=sys.stderr)
    print(f"  labels = {len(labels_sorted)}", file=sys.stderr)
    ...
    print("  top 5 labels by median z_s:", file=sys.stderr)
    for row in sorted(label_summary, key=lambda r: (-r["median_z_s"], -r["mean_z_s"], -r["n"], r["label"]))[:5]:
        print(
            f"    {row['label']}: n={row['n']}, median={row['median_z_s']:.6f}, mean={row['mean_z_s']:.6f}",
            file=sys.stderr,
        )
