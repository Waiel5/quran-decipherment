#!/usr/bin/env python3
"""H-NEW-236.1c - Targeted mufassal-short hinge injection simulator.

Pre-reg:
  findings/phase-b-hypotheses/h-new-236-1c-targeted-mufassal-hinges-prereg.md

Parent:
  H-NEW-236.1a (top-30/top-50 hinge extension)

Design:
  Keep H-NEW-236.1a's successful top-50 global hinge scaffold fixed.
  Add the top internal Juz30 consecutive jumps inside Q 78..114 at two
  small doses (+5, +10) and test whether the last surviving residual
  (R12a = L_mufassal_short excess) disappears.
"""
...
OUTPUT_JSON = PROJECT_ROOT / "findings/phase-b-hypotheses/csv/h-new-236-1c.json"
...
# Locked from the pre-reg / H-NEW-255 descriptive ranking.
JUZ30_TOP5 = [(78, 79), (79, 80), (88, 89), (83, 84), (80, 81)]
JUZ30_TOP10 = JUZ30_TOP5 + [(89, 90), (84, 85), (98, 99), (82, 83), (97, 98)]
...
def load_h2361a_top50_baseline() -> dict:
    ...
    return {
        ...
        "mufassal_empirical": float(cell["full_analysis"]["L_mufassal_short"]["empirical"]),
        "mufassal_sim_mean": float(cell["full_analysis"]["L_mufassal_short"]["sim_mean"]),
        "mufassal_gap": (
...
def juz30_internal_ranking(global_ranked: list[dict]) -> list[dict]:
    return [
        row
...
def validate_locked_internal_edges(global_ranked: list[dict]) -> list[dict]:
    internal = juz30_internal_ranking(global_ranked)
    locked = JUZ30_TOP10
    observed = [(row["a"], row["b"]) for row in internal[:10]]
    if observed != locked:
        raise AssertionError(
            f"Locked Juz30 top-10 mismatch. Expected {locked}, observed {observed}"
        )
    return internal
...
def block_chi2(empirical_obs: dict, sim_results: list[dict], rand_results: list[dict]) -> dict:
    ...
    for key in block_keys:
        ...
        empirical_stat_sim += z_sim * z_sim
        empirical_stat_rand += z_rand * z_rand
        per_block[key] = {
...
def run_cell(
    cell_name: str, seed_offset: int, hinges_1indexed: list[tuple[int, int]], dmat: list[list[float]]
) -> dict:
...
def main() -> None:
    print("=" * 72)
    print("H-NEW-236.1c - Targeted mufassal-short hinge injection")
    print("=" * 72)
    ...
    internal_ranked = validate_locked_internal_edges(ranked_edges)
    ...
    print("Empirical observables:")
    for key, value in empirical.items():
        print(f"  {key:22s} = {value:.6f}")
    print(
        "Baseline H-NEW-236.1a top-50 mufassal gap = "
        f"{baseline['mufassal_gap']:.6f}; block_chi2 = {baseline['block_chi2']:.2f}"
    )
    ...
    cell_specs = [
        ("cell_a_top50_plus_j30_top5", 0, JUZ30_TOP5),
        ("cell_b_top50_plus_j30_top10", 200_000, JUZ30_TOP10),
    ]

    cells = {}
    for cell_name, seed_offset, added_edges in cell_specs:
        hinges = top50 + [edge for edge in added_edges if edge not in top50]
        print(f"\nRunning {cell_name} with {len(hinges)} total hinges...")
        cell_run = run_cell(cell_name, seed_offset, hinges, dmat)
        ...
        rand_passes = sum(
            [
                full_analysis["L_path"]["rand_inside_95ci"],
                full_analysis["W_wrap"]["rand_inside_95ci"],
                block_stat["rand_inside_95ci"],
                full_analysis["L_tail_91_114"]["rand_inside_95ci"],
            ]
        )
        ...
        block_chi2_improvement = baseline["block_chi2"] - block_stat["empirical_stat_under_sim"]

        if full_analysis["L_mufassal_short"]["sim_inside_95ci"] and sim_passes == 4:
            verdict = "TERMINAL-HINGE-CLOSURE"
        elif mufassal_gap <= 0.5 * baseline["mufassal_gap"]:
            verdict = "PARTIAL-TERMINAL-CLOSURE"
        elif mufassal_gap < baseline["mufassal_gap"]:
            verdict = "WEAK-TERMINAL-CLOSURE"
        else:
            verdict = "R12a-PERSISTS"

        cells[cell_name] = {
            "base_scaffold": "H-NEW-236.1a top-50",
            "total_hinge_count": len(hinges),
            "hinges_1indexed": hinges,
            "added_juz30_internal_hinges_1indexed": added_edges,
            ...
            "block_chi2_improvement_vs_h2361a_top50": block_chi2_improvement,
        ...
        print(
            f"  {cell_name}: mufassal_gap={mufassal_gap:.6f}, "
            f"gap_closed={mufassal_gap_closed_pct:.2f}%, "
            f"sim_passes={sim_passes}/4, verdict={verdict}"
        )

    output = {
        "finding_id": "h-new-236-1c",
        "title": "Targeted mufassal-short hinge injection on top-50 global scaffold",
        ...
        "rules_tuple": (
            ...
            "+ M2-muq constraints + TOP-50-GLOBAL-HINGE-PRESERVATION + "
            "TOP-JUZ30-INTERNAL-HINGE-PRESERVATION for m in {5, 10})"
        ),
        "empirical": empirical,
        "baseline_h2361a_top50": baseline,
        "canonical_edge_ranking_top_100": ranked_edges[:100],
        "juz30_internal_consecutive_ranking_top_15": internal_ranked[:15],
