#!/usr/bin/env python3
"""H-NEW-236.1a - Extended hinges-constrained generative simulator.

Pre-reg:
  findings/phase-b-hypotheses/h-new-236-1a-extended-hinges-prereg.md

Parent:
  H-NEW-236.1 (top-15 hinges close 73% of the residual L_path gap)

This script extends the preserved-adjacency hinge set from top-15 to the
pre-registered top-30 and top-50 canonical Fisher-Rao consecutive edges,
while keeping the H-NEW-236 / H-NEW-236.1 simulator design otherwise fixed:

  - same classical block partition
  - same within-block simulated annealing
  - same QAC-root Fisher-Rao distance matrix (H-NEW-111)
  - same observables (L_path, W_wrap, Block-chi2, L_tail_91_114)
...
H2361_JSON = PROJECT_ROOT / "findings/phase-b-hypotheses/csv/h-new-236-1.json"
PREREG = PROJECT_ROOT / "findings/phase-b-hypotheses/h-new-236-1a-extended-hinges-prereg.md"
OUTPUT_JSON = PROJECT_ROOT / "findings/phase-b-hypotheses/csv/h-new-236-1a.json"
...
def load_h2361_gap() -> float:
    with H2361_JSON.open() as f:
        data = json.load(f)
    return float(data["h236_comparison"]["h236_1_gap_L_path"])
...
def run_cell(cell_name: str, seed_offset: int, hinges_1indexed: list[tuple[int, int]], dmat: list[list[float]]) -> dict:
...
def main() -> None:
    print("=" * 72)
    print("H-NEW-236.1a - Extended hinges-constrained simulator (top-30 + top-50)")
    print("=" * 72)
    print(f"Seed={SEED}; N_SIM={N_SIM}; N_RANDOM={N_RANDOM}; SA_ITERS={SA_ITERS}")

    dmat = load_d_matrix()
    prior_gap = load_h2361_gap()
    prereg_hash = prereg_sha256()

    ranked_edges = canonical_edge_ranking(dmat)
    top30 = [(row["a"], row["b"]) for row in ranked_edges[:30]]
    ...
    cell_specs = [
        ("cell_a_top30", 30, 0, top30),
        ("cell_b_top50", 50, 200_000, top50),
    ]
    cells = {}
    for cell_name, top_k, seed_offset, hinges in cell_specs:
        print(f"\nRunning {cell_name} with top-{top_k} hinges...")
        ...
        gap = full_analysis["L_path"]["empirical"] - full_analysis["L_path"]["sim_mean"]
        residual_closed_pct = 100.0 * (prior_gap - gap) / prior_gap
        total_h236_gap = 6.31
        total_closed_vs_h236_pct = 100.0 * (total_h236_gap - gap) / total_h236_gap
        sim_passes = sum(
            [
        ...
        if gap <= 0.017 and sim_passes == 4:
            verdict = "EQUATION-COMPLETE"
        elif gap <= 0.087 and sim_passes == 4:
            verdict = "EQUATION-COMPLETE at top-30 scale"
        elif gap < 0.5:
            verdict = "NEAR-PARTIAL-CLOSURE"
        else:
            verdict = "RESIDUAL-R12-PERSISTS"
        ...
        cells[cell_name] = {
            ...
            "l_path_gap_vs_sim_mean": gap,
            "residual_closed_pct_vs_h2361": residual_closed_pct,
            "total_closed_pct_vs_h236": total_closed_vs_h236_pct,
        ...
        print(
            f"  {cell_name}: gap={gap:.6f}, "
            f"residual_closed={residual_closed_pct:.2f}%, sim_passes={sim_passes}/4, verdict={verdict}"
        )

    output = {
        "finding_id": "h-new-236-1a",
        "title": "Extended hinges-constrained generative simulator - top-30 and top-50 cells",
        "pre_reg_sha256": prereg_hash,
        "parent": "h-new-236-1",
        "grandparent": "h-new-236 -> cross-finding-020",
        "seed": SEED,
        "n_sim": N_SIM,
        "n_random": N_RANDOM,
        "sa_iters": SA_ITERS,
        "rules_tuple": (
            ...
            "+ M2-muq constraints + TOP-K-HINGE-PRESERVATION for K in {30, 50})"
        ),
        "empirical": empirical,
        "prior_h2361_gap_l_path": prior_gap,
