#!/usr/bin/env python3
"""Build RTL Arabic PDF from the Khawātim al-Ḥashr analysis via Chrome headless."""

import subprocess, os, textwrap, re, html

ROOT = '/Users/grey/Downloads/quran'
MD_PATH = f'{ROOT}/تحليل-خواتيم-سورة-الحشر.md'
HTML_PATH = f'{ROOT}/Khawatim-al-Hashr.html'
PDF_PATH = f'{ROOT}/Khawatim-al-Hashr.pdf'

# --- minimal Markdown → HTML conversion tuned for our file ---
# (We wrote the MD, we know the constructs used.)
...
def md_to_html(md):
    ...
    def flush_table():
        ...
        if in_table and table_rows:
            out.append('<table>')
            # first row is header if second row is the |---|---| separator
            start = 0
    ...
    def inline(s):
        # escape, then restore formatting
        s = html.escape(s, quote=False)
        # bold **text**
        s = re.sub(r'\*\*([^*]+)\*\*', r'<strong>\1</strong>', s)
        # italic *text* (careful not to catch bold)
        s = re.sub(r'(?<!\*)\*(?!\*)([^*\n]+?)(?<!\*)\*(?!\*)', r'<em>\1</em>', s)
        # backtick inline code `x`
    ...
    while i < len(lines):
        ...
        if stripped.startswith('>'):
            ...
            if in_blockquote and not stripped:
                # empty line inside blockquote section — allow one, otherwise flush
                # peek ahead
        ...
        # Italic-paragraph (our footer is wrapped in *...*)
...
<head>
<meta charset="utf-8">
<title>تحليل خواتيم سورة الحشر</title>
<style>
{CSS}
</style>
</head>
...
<section class="cover">
  ...
  <h1>تحليلٌ عميقٌ<br>لخواتيم سورة الحشر</h1>
  <div class="subtitle">الآيات الأربع الأخيرة &mdash; الحشر ٢١-٢٤</div>
  <div class="ornament">❖ ✦ ❖</div>
  <div class="meta">
    دراسةٌ بحثيّةٌ تَجمع بين التحليل الحاسوبيّ<br>
    والتراث البلاغيّ والتفسيريّ الكلاسيكيّ<br>
    <br>
    ٢٠٢٦ &mdash; مشروع التحليل الشامل للقرآن الكريم
  </div>
</section>
