#!/usr/bin/env python3
"""H-NEW-1550 — Oath-opener (qasamīyāt; wa-l-* cosmic/natural-noun) whole-surah Fisher-Rao cohesion.

Pre-registration: findings/phase-b-hypotheses/prereg-h-new-1550-oath-opener-cluster.md
Classical anchor: al-Suyūṭī, al-Itqān fī ʿulūm al-Qurʾān, nawʿ 67 (al-aqsām fī l-Qurʾān).
Seed: 20260509; n_perm: 10000; α_bon: 0.025 (k=2 Bonferroni); direction one-sided LOWER (TIGHTER).
"""
...
PREREG = ROOT / "findings/phase-b-hypotheses/prereg-h-new-1550-oath-opener-cluster.md"
EXPECTED_SHA = "2ad17dab54450851c17b201b54a1bfb1191424426f63c3115639808f92ad1ac8"
FR_MATRIX = ROOT / "findings/phase-b-hypotheses/csv/h-new-111.json"
QURAN = ROOT / "quran-text/quran-no-tashkeel.json"
OUT = ROOT / "findings/phase-b-hypotheses/csv/h-new-1550.json"
SEED = 20260509
N_PERM = 10_000

# Locked cluster (strict-oath formula: v1 begins with wāw + definite article on cosmic/natural noun):
CLUSTER = [37, 51, 52, 53, 77, 79, 85, 86, 89, 91, 92, 93, 95, 100, 103]

# MW-5 PC sub-sample (H-NEW-1190 anchor; deterministic):
PC = [69, 97, 101]
...
def main():
    ...
    # ----- corpus reverify of cluster membership via strict-oath regex -----
    text = json.loads(QURAN.read_text())
    pat = re.compile(r"^\s*وال")
    # Surahs whose v1 begins with wāw + alif + lām
    found = sorted({int(s["id"]) for s in text if pat.match(s["verses"][0].get("text", ""))})
    if found != CLUSTER:
    ...
    b_pass = p_b <= 0.025
    pc_pass = p_pc <= 0.05
    reversed_dir = obs > mean(a_n)  # pre-commit violation flag

    if reversed_dir:
        verdict = "PRE-COMMIT-VIOLATION (FR > null mean — direction reversed)"
    elif not pc_pass:
    ...
    out = {
        "id": "H-NEW-1550",
        "title": "Oath-opener (qasamīyāt; wa-l-* cosmic/natural-noun) whole-surah FR cohesion",
        ...
        "cell_A_uniform": {
            ...
            "null_p2_5": sorted(a_n)[int(0.025 * len(a_n))],
            "pass": a_pass,
            "n_perm": len(a_n),
        },
        "cell_B_length_matched": {
            ...
            "null_p2_5": sorted(b_n)[int(0.025 * len(b_n))],
            "pass": b_pass,
            "n_perm": len(b_n),
            "length_window": [lo, hi],
        },
        ...
        "bonferroni_k": 2,
        "direction_locked": "LOWER (TIGHTER)",
        "verdict": verdict,
    }
