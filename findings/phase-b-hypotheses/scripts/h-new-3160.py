#!/usr/bin/env python3
"""H-NEW-3160: per-verse cross-edition exegetical divergence against the per-verse
structural profile.
...
def main():
    ...
    for nm, arr in (("C1", C1), ("C2", C2), ("C3", C3), ("C4", C4)):
        ...
        if abs(res["null_mean"] - res["null_analytic_expectation"]) > 3.0 * res["null_sd"]:
            die("%s: permutation null mean %.6g deviates from analytic expectation %.6g "
                "by more than 3 permutation-SD" % (nm, res["null_mean"],
