#!/usr/bin/env python3
"""H-NEW-1310 — Christ-narrative 3-surah cluster Fisher-Rao cohesion.

Pre-reg: findings/phase-b-hypotheses/h-new-1310-christ-narrative-cluster-prereg.md
SHA256:  d131ca234513a5bd7f825707c0778a5dedfe6f04cea6dab6e0cba7f013b17996
"""
...
PREREG = ROOT / "findings/phase-b-hypotheses/h-new-1310-christ-narrative-cluster-prereg.md"
EXPECTED_SHA = "d131ca234513a5bd7f825707c0778a5dedfe6f04cea6dab6e0cba7f013b17996"
FR_MATRIX = ROOT / "findings/phase-b-hypotheses/csv/h-new-111.json"
QURAN = ROOT / "quran-text/quran-no-tashkeel.json"
OUT = ROOT / "findings/phase-b-hypotheses/csv/h-new-1310.json"
SEED = 20260509
N_PERM = 10_000

CLUSTER = [3, 5, 19]
ADRAKA_CLUSTER = [69, 74, 77, 82, 83, 86, 90, 97, 101, 104]
...
def main() -> None:
    ...
    rng_pc_pick = random.Random(SEED)
    rng_pc_null = random.Random(SEED + 3)
    pool_no_q1 = list(range(2, 115))

    # Cell A
    a_nulls = [mean_intra(D, rng_a.sample(pool_no_q1, len(CLUSTER))) for _ in range(N_PERM)]
    p_a = sum(1 for x in a_nulls if x <= obs) / N_PERM

    # Cell B: ±20% length-matched
    target_total = sum(verse_counts[s] for s in CLUSTER)
    lo = target_total * 0.8
    hi = target_total * 1.2
    b_nulls = []
    tries = 0
    while len(b_nulls) < N_PERM and tries < N_PERM * 200:
        sample = rng_b.sample(pool_no_q1, len(CLUSTER))
    ...
    # MW-5 PC: H-NEW-1190 wa-mā adrāka mā cluster, deterministic 3-of-10 sub-sample
    pc_subsample = sorted(rng_pc_pick.sample(ADRAKA_CLUSTER, 3))
    pc_obs = mean_intra(D, pc_subsample)
    pc_nulls = [mean_intra(D, rng_pc_null.sample(pool_no_q1, 3)) for _ in range(N_PERM)]
    ...
    out = {
        "id": "H-NEW-1310",
        "title": "Christ-narrative 3-surah cluster Fisher-Rao cohesion",
        "prereg_sha": EXPECTED_SHA,
        "seed": SEED,
        "n_perm": N_PERM,
        "cluster": CLUSTER,
        "cluster_verse_counts": {s: verse_counts[s] for s in CLUSTER},
        "cluster_total_verses": target_total,
        "obs_intra_cluster_FR_mean": obs,
        "pairwise": {f"{a}-{b}": D[a][b] for i, a in enumerate(CLUSTER) for b in CLUSTER[i + 1 :]},
        "cell_A_uniform_null": {
            "n_perm": len(a_nulls),
        ...
        "MW5_positive_control": {
            "source": "H-NEW-1190 wa-mā adrāka mā cluster (CONFIRMED FR-cohesive at p=0.00068)",
            "subsample": pc_subsample,
            "pc_obs_mean": pc_obs,
    ...
    print(f"\nVerdict: {verdict}")
    print(f"Pairwise FR: {out['pairwise']}")
    print(f"Cell A uniform null: p={p_a:.5f}  null_mean={mean(a_nulls):.5f}  null_p5={sorted(a_nulls)[int(0.05*len(a_nulls))]:.5f}")
    print(f"Cell B length-matched: p={p_b:.5f}  null_p5={sorted(b_nulls)[int(0.05*len(b_nulls))]:.5f}  (n={len(b_nulls)})")
    print(f"MW-5 PC ({pc_subsample}): obs={pc_obs:.5f}  p_pc={p_pc:.5f}  pass={pc_pass}")
    print(f"Wrote: {OUT}")
